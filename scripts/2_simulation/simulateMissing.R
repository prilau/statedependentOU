...
rate = 8 / tree_length # expected number of changes is 50
...
for(i in 1:num_sim) {
  ...
  while (! (mean(history2$states == "1") > 0.25 & mean(history2$states == "2") > 0.25 ) ) {
    history2 = sim.history(tree, rate * Q2, nsim=1, message=FALSE)
  }
...
# 2 missing states
halflives <- c("1"=0.3*root_age, "2"=0.3*root_age)
